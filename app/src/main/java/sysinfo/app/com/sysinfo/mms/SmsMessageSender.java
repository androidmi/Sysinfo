...
public class SmsMessageSender implements MessageSender {
    ...
    public SmsMessageSender(Context context, String[] dests, String msgText, long threadId, int simSlot) {
        mContext = context;
        mMessageText = msgText;
        if (dests != null) {
            mNumberOfDests = dests.length;
    ...
    private boolean queueMessage(long token) throws MmsException {
        ...
        for (int i = 0; i < mNumberOfDests; i++) {
            try {
                LogTag.i(TAG, "queueMessage mDests[i]: " + mDests[i] + " mThreadId: " + mThreadId);
                Uri uri = addMessageToUri(mContext.getContentResolver(),
                ...
                LogTag.e(TAG, "queueMessage SQLiteException", e);
            }
        }
    ...
    public static Uri addMessageToUri(ContentResolver resolver,
        ...
        values.put(Telephony.Sms.READ, read ? Integer.valueOf(1) : Integer.valueOf(0));
    ...
    private String getOutgoingServiceCenter(long threadId) {
        ...
        try {
            ...
            if ((cursor == null) || !cursor.moveToFirst()) {
                LogTag.i("query smsCenter is null");
                return null;
            }
            String mmsCenter = cursor.getString(COLUMN_SERVICE_CENTER);
            int replyPath = cursor.getInt(COLUMN_REPLY_PATH_PRESENT);
            String log = "ms Center:"+mmsCenter + " reply:"+replyPath;
            LogTag.i(log);
            boolean replyPathPresent = (1 == replyPath);
            return replyPathPresent ? mmsCenter : null;
        } catch (Exception e) {
